...
VERSION 3.4 - 终极冲刺版 (冲击190分)
============================================
基于 V3.3 冠军模型 (180分) 继续微调
改进内容：
1. 超低探索率 (0.01-0.05)
2. 更精细的学习率衰减
3. 针对性起始位置训练
...
class ContinuousWorld:
    def __init__(self, world_size=12.0, friction=0.98,
                 reward_positions=None, noise_std=0.03):
        self.world_size = world_size
        self.friction = friction
        # 更多奖励点，覆盖更广区域
        if reward_positions is None:
            self.reward_positions = [3.0, -2.0, 5.0, -4.0, 1.5, -1.5, 6.0, -5.5, 0]
        else:
    ...
    def reset(self, extreme=False, target_pos=None):
        """重置环境，支持针对性训练"""
        if target_pos is not None:
            # 针对特定位置训练
            self.pos = target_pos
            self.vel = np.random.uniform(-1.0, 1.0)
        elif extreme and random.random() < 0.4:
            # 40% 概率使用极端起始位置
    ...
    def step(self, action=None):
        ...
        for rp in self.reward_positions:
            if abs(self.pos - rp) < 0.25:  # 缩小奖励半径，提高难度
                reward = 1.0
                break
...
class PredictionErrorCuriosity(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=64, lr=0.0003):
        super().__init__()
...
class NoveltyCuriosity:
    def __init__(self, state_dim, memory_size=5000, novelty_decay=0.995):
    ...
    def reset_memory(self, keep_ratio=0.8):
...
class RNDCuriosity(nn.Module):
    def __init__(self, state_dim, embedding_dim=64, hidden_dim=128, lr=0.0008):
        super().__init__()
...
class TrajectoryRecorder:
    ...
    def save(self, path='best_trajectory_v34.npy'):
...
class CuriosityConfig:
    ...
    # V3.4 高分锁定
    high_score_threshold: float = 170  # 提高阈值
    lock_duration: int = 60  # 增加锁定时间
...
class AdaptiveCuriositySystem:
    def __init__(self, config: CuriosityConfig):
        ...
        self.mechanisms = {
            'prediction_error': PredictionErrorCuriosity(
                config.state_dim, config.action_dim, config.hidden_dim, lr=0.0003),
            'novelty': NoveltyCuriosity(config.state_dim),
            'rnd': RNDCuriosity(config.state_dim, config.rnd_embedding_dim, lr=0.0008)
        }
        ...
        self.best_return = 0
        self.best_episode = 0
        self.champion_path = os.path.join(config.checkpoint_dir, "champion_model_v34.pth")
    ...
    def end_episode(self, episode_return, episode_time, details=None,
        ...
        if self.episode_count % self.config.novelty_reset_freq == 0:
            self.mechanisms['novelty'].reset_memory(keep_ratio=0.8)
...
class LongTermVisualizer:
    ...
    def setup(self):
        ...
        self.fig.suptitle('Curiosity-Driven Exploration - V3.4 (冲刺190分)', fontsize=14)
...
def train_long_term(num_episodes: int = 300,
    ...
    """V3.4 终极冲刺训练"""
    print("=" * 60)
    print("LONG-TERM CURIOSITY-DRIVEN TRAINING - VERSION 3.4")
    print("=" * 60)
    print("终极冲刺特性:")
    print("  ✅ 超低探索率 (0.01-0.05)")
    print("  ✅ 从冠军模型开始微调")
    print("  ✅ 针对性起始位置训练")
    print("  ✅ 强化高分保护 (阈值170, 锁定60)")
    print("  ✅ 缩小奖励半径 (0.25)")
    print("  ✅ 集成测试验证")
    print("=" * 60)
    ...
    explore_rate_max = 0.05
    explore_smoothing = 0.98
    recent_returns_window = 20

    # 针对性训练的位置列表
    hard_positions = [-6, -5, -4, -3, 3, 4, 5, 6, -2, 2]

    for episode in range(num_episodes):
        # V3.4 针对性起始位置训练
        if episode < 100:
            # 前100 episodes：针对性训练困难位置
        ...
        if episode < 100:
            ...
            env.set_difficulty(0.02)
        else:
            env.set_difficulty(0.015)
        ...
        if episode >= recent_returns_window and not system.is_high_score_lock_active():
            ...
            if recent_avg > 160:
                target_rate = max(explore_rate_min, explore_rate * 0.96)
            elif recent_avg > 140:
...
def analyze_results(system: AdaptiveCuriositySystem):
    ...
    plt.tight_layout()
    plt.savefig('training_analysis_v3.4.png', dpi=150)
    plt.show()
