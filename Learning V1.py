...
class ContinuousWorld:
    def __init__(self, world_size=10.0, friction=0.98,
                 reward_positions=[3.0, -2.0, 5.0], noise_std=0.05):
...
class CuriosityConfig:
    ...
    # 改进参数
    min_weight: float = 0.08  # 从 0.05 提高到 0.08（RND 权重提升）
...
class LongTermVisualizer:
    ...
    def setup(self):
        ...
        self.fig.suptitle('Curiosity-Driven Exploration - Version 3.0 (with Champion Model)', fontsize=14)
        self.initialized = True
        plt.tight_layout()
        plt.ion()
        plt.show()

    def update(self, system, current_state=None, details=None):
        ...
        if history['episode_returns']:
            ...
            ax.set_title(f'Episode Returns (Avg: {stats["avg_return"]:.1f}, Best: {stats["best_return"]})')
        ...
        if history['weights_history']:
            ...
            ax.set_title(f'Weights Evolution (min={system.config.min_weight:.0%})')
        ...
        if history.get('episode_times'):
            ax.plot(history['episode_times'], 'g-', alpha=0.7)
            ax.set_xlabel('Episode')
...
def train_long_term(num_episodes: int = 1000,
    ...
    """长期训练主函数 - v3.0 动态探索率"""
    print("=" * 60)
    print("LONG-TERM CURIOSITY-DRIVEN TRAINING - VERSION 3.0")
    print("=" * 60)
    print("新增特性:")
    print("  ✅ 动态探索率调整（根据近期表现）")
    print("  ✅ 冠军模型保存（记录最佳模型）")
    print("  ✅ 巅峰保护机制")
    print("  ✅ RND 权重提升 (min_weight=0.08)")
    print("=" * 60)
    ...
    for episode in range(start_episode, num_episodes):
        ...
        for step in range(200):
            # 【改进】动态探索率调整
            if episode >= recent_returns_window:
                ...
                if recent_avg > 120:
                    # 表现优秀，降低探索，专注利用
                    explore_rate = max(explore_rate_min, explore_rate * 0.99)
                elif recent_avg < 60:
                    # 表现不佳，增加探索
                    explore_rate = min(explore_rate_max, explore_rate * 1.02)
                else:
                    # 表现中等，缓慢衰减
                    explore_rate = max(explore_rate_min, explore_rate * 0.998)
        ...
        if visualizer and episode % 5 == 0:
            visualizer.update(system, state, episode_details)
            plt.pause(0.01)
        ...
        if (episode + 1) % verbose_freq == 0:
            ...
            print(f"Ep {episode + 1:4d}/{num_episodes} | "
                  ...
                  f"Best: {stats['best_return']} | "
                  f"Time: {elapsed / 60:.1f}min")

        # 【新增】每 50 episodes 打印一次统计摘要
        if (episode + 1) % 50 == 0:
            stats = system.get_stats()
...
def analyze_results(system: AdaptiveCuriositySystem):
    ...
    # 奖励分布直方图
    axes[0, 2].hist(returns, bins=30, edgecolor='black', alpha=0.7)
    axes[0, 2].axvline(np.mean(returns), color='r', linestyle='--', label=f'Mean: {np.mean(returns):.1f}')
    axes[0, 2].axvline(system.best_return, color='gold', linestyle='--', linewidth=2,
                       label=f'Best: {system.best_return}')
    ...
    plt.tight_layout()
    plt.savefig('training_analysis_v3.png', dpi=150)
    plt.show()
    ...
    print(f"最小权重保护: {system.config.min_weight}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    # 运行训练 - 从 Episode 500 的检查点继续
    system = train_long_term(
        num_episodes=1000,
        resume_from='checkpoints/checkpoint_ep1000.pth',  # 从 Episode 500 继续
        render=True,
        save_freq=50,
        verbose_freq=20
    )
