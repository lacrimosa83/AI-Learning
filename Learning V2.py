...
class CuriosityConfig:
    ...
    # V3.1 改进参数
    min_weight: float = 0.10  # 从 0.08 提高到 0.10
...
class AdaptiveCuriositySystem:
    def __init__(self, config: CuriosityConfig):
        ...
        self.high_score_threshold = 150
        self.lock_duration = 20
    ...
    def end_episode(self, episode_return, episode_time, details=None):
        ...
        # 【V3.1】更新高分锁定计数器
        if episode_return >= self.high_score_threshold:
            self.high_score_lock_counter = self.lock_duration
            print(f"   🔒 高分锁定激活！接下来 {self.lock_duration} episodes 保持低探索率")
...
class LongTermVisualizer:
    ...
    def setup(self):
        ...
        self.fig.suptitle('Curiosity-Driven Exploration - V3.1 (Smooth Explore Rate)', fontsize=14)
    ...
    def update(self, system, current_state=None, details=None, explore_rate=None):
        ...
        if history.get('episode_times'):
            ...
            if explore_rate is not None:
                ax_twin = ax.twinx()
                ax_twin.plot(history.get('explore_rates', []), 'b--', alpha=0.5, label='Explore Rate')
...
def train_long_term(num_episodes: int = 1500,
    ...
    """长期训练主函数 - V3.1 平滑探索率"""
    print("=" * 60)
    print("LONG-TERM CURIOSITY-DRIVEN TRAINING - VERSION 3.1")
    print("=" * 60)
    print("新增特性:")
    print("  ✅ 指数移动平均探索率（平滑调整）")
    print("  ✅ 高分锁定机制（保护高分策略）")
    print("  ✅ 探索率上限降低 (0.50 → 0.35)")
    print("  ✅ 最小权重提升 (0.08 → 0.10)")
    print("=" * 60)
    ...
    print(f"高分锁定阈值: {system.high_score_threshold} 分")
    print("-" * 60)

    total_start = time.time()

    # 【V3.1】平滑探索率参数
    explore_rate = 0.30  # 初始探索率（降低）
    explore_rate_min = 0.10
    explore_rate_max = 0.35  # 上限从 0.50 降低到 0.35
    explore_smoothing = 0.95  # 指数平滑系数
...
def analyze_results(system: AdaptiveCuriositySystem):
    ...
    axes[0, 2].set_title('Explore Rate Evolution (Smoothed)')
    ...
    plt.tight_layout()
    plt.savefig('training_analysis_v3.1.png', dpi=150)
    plt.show()
...
if __name__ == "__main__":
    ...
    # 运行训练 - 从 Episode 1000 继续到 1500
    system = train_long_term(
        num_episodes=2000,
        resume_from='checkpoints/checkpoint_ep1000.pth',  # 从 Episode 1000 继续
        render=True,
        save_freq=50,
        verbose_freq=20
    )
