...
def setup_device():
    if torch.cuda.is_available():
        ...
        # 显示 GPU 详细信息
        print("\n📊 GPU 状态:")
        print(f"   内存分配: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
        print(f"   内存缓存: {torch.cuda.memory_reserved() / 1e9:.2f} GB")
        print("=" * 60)
    else:
...
if torch.cuda.is_available():
    ...
    torch.backends.cudnn.benchmark = True  # 优化 GPU 性能
...
class World2DWithDynamicObstacles:
    ...
    def reset(self, extreme=True):
        ...
        corners = [
            ...
            (self.world_size / 2 - 0.5, -self.world_size / 2 + 0.5),
            (self.world_size / 2 - 0.5, self.world_size / 2 - 0.5),
            (-self.world_size / 2 + 1, 0),
            (self.world_size / 2 - 1, 0),
            (0, -self.world_size / 2 + 1),
            (0, self.world_size / 2 - 1),
        ]
...
class NoveltyCuriosity2D:
    def __init__(self, state_dim=4, memory_size=20000, novelty_decay=0.998):
...
class CuriosityConfigV42:
    ...
    save_freq: int = 100
    checkpoint_dir: str = "checkpoints_v42"
    min_weight: float = 0.10
    novelty_reset_freq: int = 500
    exploration_bonus: float = 0.08
    weight_reset_threshold: float = 0.95
    high_score_threshold: float = 120
    lock_duration: int = 80


class AdaptiveCuriositySystemV42:
    def __init__(self, config: CuriosityConfigV42):
        self.config = config
        ...
        self.champion_path = os.path.join(config.checkpoint_dir, "champion_model_v42.pth")
    ...
    def load_checkpoint(self, path):
        ...
        self.mechanisms['novelty'].memory = deque(checkpoint['novelty_memory'], maxlen=20000)
...
class Visualizer2DWithObstacles:
    ...
    def setup(self):
        ...
        self.fig.suptitle('2D Exploration with Dynamic Obstacles - V4.2', fontsize=14, fontweight='bold')
    ...
    def update_trajectory(self, ax, positions, reward_zones, static_obstacles, dynamic_obstacles, current_pos=None):
        ...
        ax.set_title('2D Trajectory with Dynamic Obstacles')
...
def train_v42_full(num_episodes: int = 5000,
                   resume_from: str = None,
                   render: bool = True,
                   save_freq: int = 100,
                   verbose_freq: int = 20):
    """V4.2 满血训练 - 5000 episodes，充分利用 GPU"""
    print("=" * 60)
    print("V4.2 FULL POWER TRAINING - 5000 EPISODES")
    print("=" * 60)
    print("环境配置:")
    print(f"  ✅ 奖励区域: 4 个 (半径 0.4)")
    print(f"  ✅ 静态障碍物: 12 个")
    print(f"  ✅ 动态障碍物: 4 个")
    print(f"  ✅ 碰撞惩罚: -1.0")
    ...
    config = CuriosityConfigV42()
    env = World2DWithDynamicObstacles()
    system = AdaptiveCuriositySystemV42(config)
    ...
    print(f"设备: {DEVICE}")
    print(f"网络隐藏层维度: {config.hidden_dim}")
    print(f"保存频率: 每 {save_freq} episodes")
    print(f"输出频率: 每 {verbose_freq} episodes")
    print("-" * 60)
    ...
    for episode in range(start_episode, num_episodes):
        ...
        # 每 200 episodes 打印 GPU 状态
        if (episode + 1) % 200 == 0 and torch.cuda.is_available():
            print(f"\n   📊 GPU 状态 (Ep {episode + 1}):")
            print(f"      内存分配: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
            print(f"      内存缓存: {torch.cuda.memory_reserved() / 1e9:.2f} GB")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("配置:")
    print("  - 训练量: 5000 episodes")
    print("  - 静态障碍物: 12 个")
    print("  - 动态障碍物: 4 个")
    print("  - 奖励区域: 4 个 (半径 0.4)")
    print("  - 碰撞惩罚: -1.0")
    print("  - 网络规模: 256 隐藏单元")
    print("=" * 60)

    # 检查是否有之前的检查点
    checkpoint_path = "checkpoints_v42/champion_model_v42.pth"
