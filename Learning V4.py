...
class ContinuousWorld:
    def __init__(self, world_size=12.0, friction=0.98,  # 扩大边界
                 reward_positions=None, noise_std=0.05):
        self.world_size = world_size
        self.friction = friction
        # V3.3 更多奖励点，覆盖更广区域
        if reward_positions is None:
            self.reward_positions = [3.0, -2.0, 5.0, -4.0, 1.5, -1.5, 6.0, -5.5]
        else:
    ...
    def reset(self, extreme=False):
        """重置环境，支持极端起始位置"""
        if extreme and random.random() < 0.3:
            # 30% 概率使用极端起始位置
...
class PredictionErrorCuriosity(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=64, lr=0.0005):  # 降低学习率
        super().__init__()
...
class NoveltyCuriosity:
    ...
    def reset_memory(self, keep_ratio=0.7):  # 保留更多记忆
...
class RNDCuriosity(nn.Module):
    def __init__(self, state_dim, embedding_dim=64, hidden_dim=128, lr=0.001):  # 降低学习率
        super().__init__()
...
class TrajectoryRecorder:
    ...
    def save(self, path='best_trajectory.npy'):
    ...
    def analyze(self):
        ...
        print(
            f"      动作分布: {np.bincount([a + 1 if a is not None else 0 for a in self.best_trajectory['actions']])}")
...
class CuriosityConfig:
    ...
    learning_rate: float = 0.005  # 降低学习率
    weight_update_freq: int = 10
    credit_gamma: float = 0.95
    save_freq: int = 50
    checkpoint_dir: str = "checkpoints"
    # V3.3 参数
    min_weight: float = 0.12  # 提高最小权重
    novelty_reset_freq: int = 150  # 降低重置频率
    exploration_bonus: float = 0.02  # 降低探索保底
    weight_reset_threshold: float = 0.95
    # V3.3 高分锁定
    high_score_threshold: float = 160  # 提高阈值
    lock_duration: int = 50  # 增加锁定时间
...
class AdaptiveCuriositySystem:
    def __init__(self, config: CuriosityConfig):
        ...
        self.mechanisms = {
            ...
            'rnd': RNDCuriosity(config.state_dim, config.rnd_embedding_dim, lr=0.001)
        }
        ...
        self.best_return = 0
        self.best_episode = 0
        self.champion_path = os.path.join(config.checkpoint_dir, "champion_model_v33.pth")
        ...
        for i in range(num_tests):
            ...
            for step in range(200):
                if use_greedy:
                    ...
                    # 使用模型的好奇心选择动作
                    next_state, _, _ = env.step(None)  # 临时
...
class LongTermVisualizer:
    ...
    def setup(self):
        ...
        self.fig.suptitle('Curiosity-Driven Exploration - V3.3 (Ultimate Refinement)', fontsize=14)
...
def train_long_term(num_episodes: int = 500,
                    resume_from: str = None,
                    load_champion_first: bool = True,
                    render: bool = True,
                    save_freq: int = 50,
                    verbose_freq: int = 20):
    """V3.3 终极精炼训练"""
    print("=" * 60)
    print("LONG-TERM CURIOSITY-DRIVEN TRAINING - VERSION 3.3")
    print("=" * 60)
    print("终极精炼特性:")
    print("  ✅ 极低探索率 (0.02-0.10)")
    print("  ✅ 从冠军模型开始微调")
    print("  ✅ 增强起始位置多样性")
    print("  ✅ 优化高分锁定 (阈值160, 锁定50)")
    print("  ✅ 记录最佳轨迹")
    print("  ✅ 渐进式噪声衰减")
    print("=" * 60)
    ...
    # V3.3 极低探索率参数
    explore_rate = 0.08  # 初始探索率极低
    explore_rate_min = 0.02
    explore_rate_max = 0.10
    ...
    for episode in range(start_episode, start_episode + num_episodes):
        ...
        episode_progress = episode / (start_episode + num_episodes)

        # V3.3 渐进式噪声衰减
        if episode < 200:
            env.set_difficulty(0.05)
        elif episode < 400:
            env.set_difficulty(0.04)
        else:
            env.set_difficulty(0.03)
        ...
        if episode >= recent_returns_window and not system.is_high_score_lock_active():
            ...
            if recent_avg > 150:
                target_rate = max(explore_rate_min, explore_rate * 0.95)
            elif recent_avg > 120:
        ...
        if (episode + 1) % verbose_freq == 0:
            ...
            print(f"Ep {episode + 1:4d}/{start_episode + num_episodes} | "
        ...
        if (episode + 1) % 50 == 0:
            ...
            print(f"   📊 统计摘要: Avg Return={stats['avg_return']:.1f}, "
                  f"Best={stats['best_return']}, Explore={explore_rate:.3f}, "
                  f"Noise={env.noise_std:.2f}")
...
def analyze_results(system: AdaptiveCuriositySystem):
    ...
    plt.tight_layout()
    plt.savefig('training_analysis_v3.3.png', dpi=150)
    plt.show()
...
if __name__ == "__main__":
    ...
    system = train_long_term(
        num_episodes=500,  # 微调 500 episodes
        load_champion_first=True,  # 从冠军模型开始
        render=True,
        save_freq=50,
        verbose_freq=20
    )
